                    ...
                    $('[data-refreshshow]').on('click change', function (e) {
                        ...
                        switch (data) {
                            ...
                            case'popular':
                                ...
                                break;
                            case'genre':
                                getData('/books/gonre/' + $this.data('refreshid'), {
                                    offset: function(){
                                        var oldOffset = $this.data('refreshoffset');
                                        if(oldOffset < 1){
                                            $this.data('refreshoffset', ++oldOffset);
        ...
        var ProductCard = function () {
            ...
            return {
                init: function () {
                    ...
                    $ProductAssesment.find('.Rating input').on('change',
                        function (e) {
                        var $this = $(this), check = $this.prop('checked');

                        Login().postData('books/changeBookStatus/'+$('.Book_Id').attr('bookId')+"/"+$('.Book_Rating').attr('bookRating'),
                            {
                            bookId: $('.Book_Id').attr('bookId'),
                            ...
                            var $status = document.cookie.split('; ').find(row=>row.startsWith('statusRating')).split('=')[1];//запрос данных cookie - statusRating
                            //alert($status);
                            var $title = $this.closest('.Rating').find('.Rating-title');
                            if ($status == 1) {
                                $title.addClass('Rating-title_success').text('Рейтинг успешно изменен');
                            }else if($status == 0){
                                $title.addClass('Rating-title_error').text('Оценка не сохранена! Оценивать книгу может только зарегистрированный пользователь!');
                            }
                            else {
